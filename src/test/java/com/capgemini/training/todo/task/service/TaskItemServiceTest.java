...
@ContextConfiguration(classes = { TodoAppApplication.class, SecurityConfiguration.class })
public class TaskItemServiceTest {
    ...
    public void findTaskItem_ShouldReturnTaskItem() throws Exception {
        ...
        mockMvc.perform(get("/task-item/{id}", taskId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .with(user("user")))
    ...
    public void saveTaskItem_ShouldSaveAndReturnTaskItem() throws Exception {
        ...
        mockMvc.perform(post("/task-item/")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(newTaskItemJson)
                        .with(user("user").roles(AccessControl.ROLE_MAINTAINER)))
    ...
    public void deleteTaskList_ShouldDeleteTaskItem() throws Exception {
        ...
        mockMvc.perform(delete("/task-item/{id}", taskId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .with(user("user").roles(AccessControl.ROLE_MAINTAINER)))
                .andExpect(status().isOk());
